    ...
    for node, children in graph.items():
        ...
        for child in children:
            if child not in result:
                ...
                result[child] += 1  # {'A': 0, 'B': 1, 'C': 2, 'D': 2, 'E': 1, 'F': 2}
    return result
...
def find_node_without_dependencies(dependencies_by_node):
    for node, dependencies in dependencies_by_node.items():
        if dependencies == 0:
            return node
    return None          # ако е None, значи има цикъл


nodes = int(input())
graph = {}  # ще работим с dictionary, а не с list, защото въвежданите стойности са strings, а не int
